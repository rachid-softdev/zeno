...
export function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const { error } = await signUp(email, 'password123', name, agency);
    setLoading(false);
    if (error) {
      toast.error(error);
    } else {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-bg-base">
        <div className="w-full max-w-sm">
          ...
          <button className="w-full py-2.5 border border-border-subtle hover:border-border-active rounded-lg text-text-secondary text-sm font-medium transition-colors flex items-center justify-center gap-2">
